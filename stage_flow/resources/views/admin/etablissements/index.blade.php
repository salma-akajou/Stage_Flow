...
@section('title', 'Gestion des Établissements - StageFlow')
@section('breadcrumb', 'Établissements')
...
<div x-data="{
    ...
    submitSearch() {
        fetch('{{ route('admin.etablissements.index') }}?search=' + this.search, {
            headers: { 'X-Requested-With': 'XMLHttpRequest' }
        })
        .then(res => res.text())
        .then(html => {
            document.getElementById('etablissements-table-container').innerHTML = html;
        });
    }
}">
    ...
    <!-- Conteneur Tableau -->
    <div id="etablissements-table-container" class="relative z-10" data-aos="fade-up">
        @include('components.admin.etablissements.table', ['etablissements' => $etablissements])
    </div>
    ...
    <div x-show="openModal" class="fixed inset-0 z-[110] transition-opacity duration-300" style="display: none;">
        ...
        <div class="absolute inset-0 flex flex-col items-center justify-center p-4 z-10 pointer-events-none">
            <div class="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl flex flex-col pointer-events-auto transform transition-all duration-300 border border-gray-100 overflow-hidden relative p-8">
                ...
                <div class="mb-6">
                    <h3 class="text-2xl font-black text-gray-900 font-heading tracking-tight" x-text="editMode ? 'Modifier l\'établissement' : 'Ajouter un établissement'">...</h3>
                    <p class="text-sm text-gray-500 mt-1" x-text="editMode ? 'Modifiez le nom de l\'établissement existant.' : 'Remplissez les informations pour ajouter un nouvel établissement.'">...</p>
                </div>

                <form :action="editMode ? '{{ url('admin/etablissements') }}/' + itemId : '{{ route('admin.etablissements.store') }}'" method="POST">
                    @csrf
