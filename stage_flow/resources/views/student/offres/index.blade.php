...
<div class="space-y-10" x-data="{ 
    search: '{{ request('titre') }}',
    ville: '{{ request('ville_id') }}',
    secteur: '{{ request('secteur') }}'
}">
    <div class="bg-white border border-gray-200 rounded-xl p-4 sm:p-6 shadow-sm" data-aos="fade-up">
        <form action="{{ route('offres.index') }}" method="GET" class="flex flex-col lg:flex-row items-center gap-4">
            ...
            <div class="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
                <select name="ville_id" x-model="ville" @change="$el.form.submit()"
                    class="py-3 px-4 pe-9 block w-full sm:w-48 border-gray-200 rounded-lg text-sm font-medium text-gray-500 focus:border-indigo-500 focus:ring-indigo-500">
                ...
                <select name="secteur" x-model="secteur" @change="$el.form.submit()"
                    class="py-3 px-4 pe-9 block w-full sm:w-48 border-gray-200 rounded-lg text-sm font-medium text-gray-500 focus:border-indigo-500 focus:ring-indigo-500">
