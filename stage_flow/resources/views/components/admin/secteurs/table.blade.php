<div class="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
                ...
                <div class="overflow-hidden">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th scope="col" class="px-6 py-4 text-start text-xs font-semibold text-gray-500 uppercase tracking-wider">Nom du secteur</th>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50/80 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-bold text-gray-800">{{ $secteur->nom }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-medium">
                                    {{ $secteur->created_at->translatedFormat('d M Y') }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                                    <div class="flex items-center justify-end gap-x-1">
                                        @can('gerer-utilisateurs')
                                            <button type="button" @click="openEdit({{ $secteur->id }}, '{{ addslashes($secteur->nom) }}')" class="p-2 inline-flex justify-center items-center rounded-lg text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 transition" title="Modifier">
                                            ...
                                            <form action="{{ route('admin.secteurs.destroy', $secteur->id) }}" method="POST" class="inline" onsubmit="return confirm('Supprimer définitivement ce secteur ?')">
                                                @csrf
                                                @method('DELETE')
                            ...
                            <tr>
                                <td colspan="3" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <p class="text-gray-500 font-medium">Aucun secteur trouvé.</p>
                                    </div>
                                </td>
                            </tr>
                            @endforelse
    ...
    @if($secteurs->hasPages())
        <div class="px-6 py-4 border-t border-gray-200">
            {{ $secteurs->links('components.pagination') }}
        </div>
    @endif
</div>
