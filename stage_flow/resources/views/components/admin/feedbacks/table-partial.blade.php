    ...
    @forelse($feedbacks as $feedback)
        ...
        <div class="p-6 hover:bg-gray-50 transition-all group">
            <div class="flex flex-col sm:flex-row sm:items-start gap-4">
                <div class="flex items-start gap-4 flex-1">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex flex-wrap items-center gap-x-2 gap-y-1 mb-1">
                            <p class="font-semibold text-gray-900 text-sm">{{ $feedback->auteur->prenom }} {{ $feedback->auteur->nom }}</p>
                            <span class="py-0.5 px-2 {{ $feedback->auteur->role === 'etudiant' ? 'bg-blue-100 text-blue-700' : 'bg-indigo-100 text-indigo-700' }} text-[9px] font-medium rounded-full">
                        ...
                        <div class="flex flex-wrap items-center gap-x-4 gap-y-2">
                            <span class="text-xs text-{{ $badgeColor }}-600 font-medium flex items-center gap-1.5">
                                {!! $badgeIcon !!}
                                {{ $badgeText }}
                            </span>
                ...
                <div class="flex items-center justify-end gap-2 pt-3 sm:pt-0 border-t border-gray-50 sm:border-t-0 shrink-0">
                    @if(!$feedback->valide)
                        <form action="{{ route('admin.feedbacks.approve', $feedback->id) }}" method="POST" class="m-0">
                            @csrf
                            <button type="submit" class="p-2.5 text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-xl border border-gray-100 sm:border-0 transition" title="Approuver">
                    ...
                    @endif
                    <form action="{{ route('admin.feedbacks.destroy', $feedback->id) }}" method="POST" onsubmit="return confirm('Supprimer ce commentaire ?')" class="m-0">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="p-2.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-xl border border-gray-100 sm:border-0 transition" title="Supprimer">
