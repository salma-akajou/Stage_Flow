    ...
    <div class="flex flex-col md:flex-row gap-10 items-start relative z-10">
        <div class="flex flex-col items-center gap-4">
            <div class="relative group">
                <div class="size-32 rounded-3xl bg-indigo-50 flex items-center justify-center text-indigo-600 font-bold text-2xl overflow-hidden border-4 border-white shadow-xl group-hover:shadow-indigo-100 transition duration-300">
                    @if($entreprise->logo)
                        <img id="logo-preview" src="{{ asset('storage/'.$entreprise->logo) }}" class="size-full object-cover">
                    @else
                        <span id="logo-placeholder" class="text-4xl">{{ substr($entreprise->nom_entreprise, 0, 1) }}</span>
                        <img id="logo-preview" class="size-full object-cover hidden">
                    @endif
                </div>
        ...
        <div class="flex-1 w-full grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="relative" x-data="{ 
                open: false, 
                selected: '{{ $entreprise->secteur }}',
                options: ['Informatique', 'Design', 'Marketing', 'Commerce', 'Industrie', 'Autre']
            ...
            <div class="relative" x-data="{ 
                open: false, 
                selectedId: '{{ $entreprise->ville_id }}',
                selectedNom: '{{ $entreprise->ville->nom ?? "Choisir une ville" }}',
                villes: @js($villes)
            }" :class="{ 'z-[70]': open, 'z-[60]': !open }">
