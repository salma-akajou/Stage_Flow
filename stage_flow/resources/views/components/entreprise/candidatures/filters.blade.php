    ...
    <form @submit.prevent="fetchCandidatures()" class="flex flex-col lg:flex-row items-center gap-4 w-full m-0">
        <div class="relative w-full lg:flex-1 min-w-[200px]">
            ...
            <input type="text" x-model="search" @input.debounce.500ms="fetchCandidatures()" placeholder="Rechercher un candidat..."
                class="py-2.5 px-3 ps-10 block w-full border-slate-200 rounded-xl text-sm focus:border-indigo-500 focus:ring-indigo-500 border bg-white shadow-none transition">
        </div>

        <div class="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto lg:ml-auto">
            <div x-data="dropdownFilter" class="relative w-full sm:w-auto inline-flex">
                <button @click="toggle()" @click.away="close()" type="button" 
                    class="w-full sm:w-auto py-2.5 px-4 inline-flex items-center justify-between sm:justify-start gap-x-2 text-sm font-medium rounded-xl border border-slate-200 bg-white text-gray-700 shadow-none hover:bg-gray-50 focus:outline-none transition-all">
                ...
                <div x-show="open" x-transition.opacity style="display: none;" class="absolute top-full left-0 sm:left-auto sm:right-0 mt-2 z-[60] bg-white shadow-xl rounded-xl p-2 border border-gray-100 w-full sm:min-w-[160px]">
            ...
            <div x-data="dropdownFilter" class="relative w-full sm:w-auto inline-flex">
                ...
                <div x-show="open" x-transition.opacity style="display: none;" class="absolute top-full left-0 sm:left-auto sm:right-0 mt-2 z-[60] bg-white shadow-xl rounded-xl p-2 border border-gray-100 w-full sm:min-w-[200px]">
