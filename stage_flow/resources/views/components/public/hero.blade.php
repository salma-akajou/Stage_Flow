        ...
        <div class="grid lg:grid-cols-2 gap-10 items-center">
            <div class="space-y-4" data-aos="fade-right">
                ...
                <div class="flex flex-wrap gap-4 pt-2">
                    @php
                        $aventureRoute = route('register');
                        if(auth()->check()) {
                            $aventureRoute = match(auth()->user()->getRoleNames()->first()) {
                    ...
                    @endphp
                    <a href="{{ $aventureRoute }}" class="py-4 px-8 inline-flex items-center gap-x-2 text-sm font-bold rounded-xl border border-transparent bg-indigo-600 text-white hover:bg-indigo-700 shadow-xl shadow-indigo-100 transition-all hover:scale-105 active:scale-95 group">
