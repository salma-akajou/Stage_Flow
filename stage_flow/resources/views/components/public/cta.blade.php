<section class="py-24 px-4 overflow-hidden" data-aos="fade-up">
    <div class="max-w-4xl mx-auto relative bg-indigo-600 rounded-[3rem] p-12 lg:p-20 text-center shadow-2xl shadow-indigo-200 overflow-hidden">
        ...
        @php
            ...
            if(auth()->check()) {
                $ctaRoute = match(auth()->user()->getRoleNames()->first()) {
        ...
        @endphp
        <a href="{{ $ctaRoute }}" class="py-5 px-12 inline-block bg-white text-indigo-600 font-bold rounded-2xl shadow-xl hover:bg-slate-50 hover:shadow-2xl transition-all hover:scale-105 active:scale-95 leading-none">
            {{ auth()->check() ? 'Accéder à mon espace' : 'Créer mon compte maintenant' }}
        </a>
