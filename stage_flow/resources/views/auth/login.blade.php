...
<head>
    ...
    <title>Connexion - StageFlow</title>
...
<body class="bg-[#fafbfc] min-h-full flex flex-col items-center justify-center relative overflow-x-hidden py-12">
    ...
    <div class="fixed inset-0 pointer-events-none z-0">
        ...
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 size-[500px] bg-indigo-400/5 rounded-full blur-[100px]"></div>
    </div>
    ...
    <main class="w-full max-w-md p-6 relative z-10" data-aos="zoom-in">
        ...
        <div class="bg-white rounded-[2.5rem] shadow-2xl shadow-indigo-100/50 border border-slate-100 p-8 lg:p-10 relative overflow-hidden">
            ...
            <div class="text-center mb-10 relative z-10">
                <h1 class="text-2xl font-black font-heading text-slate-900 mb-2">Bon retour ! 👋</h1>
            ...
            <form action="{{ route('login') }}" method="POST" class="space-y-5">
                ...
                <!-- Email -->
                <div>
                    <label for="email" class="block text-xs font-bold uppercase tracking-widest text-slate-400 mb-2 ml-1">Email professionnel</label>
                    <input type="email" id="email" name="email" value="{{ old('email') }}" class="py-3 px-5 block w-full border-slate-200 rounded-2xl text-sm focus:border-indigo-500 focus:ring-indigo-500 transition-all font-medium" placeholder="nom@exemple.com" required autofocus>
                ...
                <div>
                    <div class="flex justify-between items-center mb-2">
                        <label for="password" class="block text-xs font-bold uppercase tracking-widest text-slate-400 ml-1">Mot de passe</label>
                        @if (Route::has('password.request'))
                            <a class="text-xs font-bold text-indigo-600 hover:text-indigo-700 focus:outline-none" href="{{ route('password.request') }}">Oublié ?</a>
                        @endif
                    </div>
                    <input type="password" id="password" name="password" class="py-3 px-5 block w-full border-slate-200 rounded-2xl text-sm focus:border-indigo-500 focus:ring-indigo-500 transition-all font-medium" placeholder="••••••••" required>
                    @error('password') <p class="text-[10px] text-rose-500 font-bold mt-1 ml-1">{{ $message }}</p> @enderror
                </div>

                <!-- Remember Me -->
                <div class="flex items-center">
                    <input id="remember" name="remember" type="checkbox" {{ old('remember') ? 'checked' : '' }} class="shrink-0 border-slate-300 rounded text-indigo-600 focus:ring-indigo-500">
                    <label for="remember" class="ms-3 text-sm font-medium text-slate-600">Se souvenir de moi</label>
                </div>
            ...
            <p class="mt-8 text-center text-sm font-medium text-slate-500 border-t border-slate-50 pt-6">
                ...
                <a class="text-indigo-600 font-bold hover:text-indigo-700 transition" href="{{ route('register') }}">
                    S'inscrire
                </a>
            </p>
        </div>
